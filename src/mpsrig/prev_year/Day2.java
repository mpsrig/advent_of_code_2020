...
public class Day2 {
    public static List<Integer> parseProgram(String program) {
    ...
    public static void runProgram(List<Integer> program) {
        for (int pc = 0; ; pc += 4) {
            ...
            int x = program.get(program.get(pc + 1));
            int y = program.get(program.get(pc + 2));
            int destIdx = program.get(pc + 3);
            int result;
            if (opcode == 1) {
                // ADD
                result = x + y;
    ...
    public static void doPart2(String puzzleInput) {
        for (int noun = 0; noun < 100; noun++) {
            for (int verb = 0; verb < 100; verb++) {
                int result = runPuzzleWithParams(puzzleInput, noun, verb);
                if (result == 19690720) {
    ...
    public static int runPuzzleWithParams(String puzzleInput, int noun, int verb) {
        List<Integer> parsedProgram = parseProgram(puzzleInput);
        parsedProgram.set(1, noun);
        parsedProgram.set(2, verb);
